...
def active_risk_report(portfolio, snapshot):
    ...
    idiosyncratic_var = 0.0 if pd.isna(active_idiosyncratic) else float(active_idiosyncratic) ** 2
    te2 = float(report["variance_contribution"].sum()) + idiosyncratic_var
    ...
    return {
        ...
        "idiosyncratic_contribution": contribution(idiosyncratic_var, tracking_error),
...
def idiosyncratic_by_name(portfolio, snapshot):
    ...
    risks = snapshot.idiosyncratic_risk.set_index("ticker")["idiosyncratic_risk"].reindex(weights.index)
    ...
    return {
        "rows": names,
        "total_idiosyncratic_risk": float(np.sqrt(total)),
        "top_name_share": names[0]["idiosyncratic_variance_share"] if names else None,
...
def idiosyncratic_overlap(left, right, snapshot):
    ...
    """
    risks = snapshot.idiosyncratic_risk.set_index("ticker")["idiosyncratic_risk"] ** 2
