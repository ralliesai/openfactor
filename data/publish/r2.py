...
class R2Client:
    """Tiny Cloudflare R2 client for OpenFactor data files.

    Example:
        client = R2Client.from_env()
        client.upload_text("a,b\n1,2\n", "openfactor-public", "x.csv", "text/csv")
        writes one object using OPENFACTOR_R2_* credentials.
    """
    ...
    def upload_text(self, text, bucket, key, content_type="text/plain; charset=utf-8"):
        ...
        Example:
            client.upload_text("a,b\n1,2\n", "openfactor-public", "x.csv", "text/csv")
            stores a CSV object without writing a local file first.
        """
    ...
    def list_keys(self, bucket, prefix=""):
        ...
        Example:
            client.list_keys("openfactor-public", "factors/openfactor-us1000")
            returns uploaded factor CSV keys.
        """
        keys = []
        token = None
        while True:
    ...
    def delete_key(self, bucket, key):
        ...
        Example:
            client.delete_key("openfactor-public", "factors/x/latest.json")
            removes that object from R2.
        """
    ...
    def delete_prefix(self, bucket, prefix=""):
        ...
        Example:
            client.delete_prefix("openfactor-public", "factors/openfactor-us1000")
            removes all US1000 public files.
        """
    ...
    def request(
        ...
        Example:
            client.request("PUT", "openfactor-public", "hello.txt", b"hello", "text/plain")
            stores hello.txt in the bucket.
        """
        path = f"/{quote(bucket)}"
        if key:
    ...
    def signing_key(self, datestamp):
        """Return the AWS SigV4 signing key for one date.

        Example:
            client.signing_key("20260616")
            returns bytes used to sign R2 requests.
        """
        key = hmac.new(
            ("AWS4" + self.secret_access_key).encode(),
            datestamp.encode(),
            hashlib.sha256,
        ).digest()
